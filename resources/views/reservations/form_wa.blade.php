...
.content-container {
    ...
    .whatsapp-btn:hover {
        background-color: #128C7E;  /* より濃い色に */
        color: white;
    }
</style>
</head>
...
@if($status > 1)
    ...
    <div class="content-container">
        ...
        <p>
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('reservation.confirm') }}" class="needs-validation" novalidate>
    @csrf
    <!-- Date -->
    <div class="mb-4">
    ...
    <div class="mb-4">
        ...
        <select class="form-select @error('time') is-invalid @enderror" id="time" name="time" required>
            ...
            @foreach(config('reservation_kitano.available_times') as $time)
        ...
        @error('time')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
        <small class="text-muted">{{ config('reservation_kitano.restaurant_info.open_hours') }}</small>
    </div>
    ...
    <div class="mb-4">
        ...
        <select class="form-select @error('guests') is-invalid @enderror" id="guests" name="guests" required>
            <option value="">Please select</option>
            @for($i = config('reservation_kitano.min_guests'); $i <= config('reservation_kitano.max_guests'); $i++)
        ...
        @error('guests')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
        <small class="text-muted">For parties larger than {{ config('reservation_kitano.max_guests') }}, please call us directly</small>
    </div>
<!-- Submit Button -->
<div class="text-center">
    <button type="button" id="whatsappReservationBtn" class="btn btn-lg px-5" style="background-color: #25D366; color: white; border: none; transition: background-color 0.3s ease;">
        <i class="bi bi-whatsapp me-2"></i> Reserve via WhatsApp
    </button>
</div>
</form>
                    </div>
...
$(document).ready(function() {
    ...
    maxDate.setDate(today.getDate() + {{ config('reservation_kitano.booking_end_days') }});
...
// wa
$(document).ready(function() {
    // 既存の日付バリデーション処理はそのまま維持
    
    // WhatsApp予約ボタンのハンドラー
    $('#whatsappReservationBtn').on('click', function() {
        // フォームデータの取得
        const date = $('#date').val();
        const time = $('#time').val();
